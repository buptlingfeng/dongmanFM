...
public class CollapsingAvatarToolbar extends LinearLayout implements AppBarLayout.OnOffsetChangedListener {
    ...
    public CollapsingAvatarToolbar(Context context, AttributeSet attrs) {
        ...
        TypedArray a = context.getTheme().obtainStyledAttributes(attrs, com.dongman.fm.R.styleable.CollapsingAvatarToolbar, 0, 0);

        try {
            collapsedPadding = a.getDimension(com.dongman.fm.R.styleable.CollapsingAvatarToolbar_collapsedPadding, -1);
            expandedPadding = a.getDimension(com.dongman.fm.R.styleable.CollapsingAvatarToolbar_expandedPadding, -1);

            collapsedImageSize = a.getDimension(com.dongman.fm.R.styleable.CollapsingAvatarToolbar_collapsedImageSize, -1);
            expandedImageSize = a.getDimension(com.dongman.fm.R.styleable.CollapsingAvatarToolbar_expandedImageSize, -1);

            collapsedTextSize = a.getDimension(com.dongman.fm.R.styleable.CollapsingAvatarToolbar_collapsedTextSize, -1);
            expandedTextSize = a.getDimension(com.dongman.fm.R.styleable.CollapsingAvatarToolbar_expandedTextSize, -1);
        } finally {
            a.recycle();
        }
        ...
        if (collapsedPadding < 0) {
            collapsedPadding = resources.getDimension(com.dongman.fm.R.dimen.default_collapsed_padding);
        }
        if (expandedPadding < 0) {
            expandedPadding = resources.getDimension(com.dongman.fm.R.dimen.default_expanded_padding);
        }
        if (collapsedImageSize < 0) {
            collapsedImageSize = resources.getDimension(com.dongman.fm.R.dimen.default_collapsed_image_size);
        }
        if (expandedImageSize < 0) {
            expandedImageSize = resources.getDimension(com.dongman.fm.R.dimen.default_expanded_image_size);
        }
        if (collapsedTextSize < 0) {
            collapsedTextSize = resources.getDimension(com.dongman.fm.R.dimen.default_collapsed_text_size);
        }
        if (expandedTextSize < 0) {
            expandedTextSize = resources.getDimension(com.dongman.fm.R.dimen.default_expanded_text_size);
        }
    }
    ...
    private View findAvatar() {
        View avatar = this.findViewById(com.dongman.fm.R.id.cat_avatar);
        if (avatar == null) {
    ...
    private TextView findTitle() {
        TextView title = (TextView) this.findViewById(com.dongman.fm.R.id.cat_title);
        if (title == null) {
